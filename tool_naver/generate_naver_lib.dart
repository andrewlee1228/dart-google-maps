...
Future<void> main() async {
  const docBaseUrl = 'https://navermaps.github.io/maps.js.ncp/docs';
  ...
  await Process.run(
    'dart',
    ['fix', '--apply', 'lib/src/generated_naver'],
    workingDirectory: projectHome,
  );
  await Process.run(
    'dart',
    ['format', 'lib/src/generated_naver'],
    workingDirectory: projectHome,
  );
}
...
Future<void> generatePrivateLibs(
  ...
  // collect libraries
  final response = await http.get(Uri.parse('$docBaseUrl/index.html'));
  final document = parse(response.body);
  final libPages = document.querySelectorAll('nav > ul > li > a').map((e) {
    return (
      id: e.text.replaceAll('.', '_'),
      title: e.text,
  ...
  // generate part files
  final genFolder = '$projectHome/lib/src/generated_naver';
...
Future<void> generatePublicLibs(
  ...
  final libFile = File('$projectHome/lib/naver_maps.dart');
  final genFolder = '$projectHome/lib/src/generated_naver';
  final privateLibs = await Directory(genFolder)
      .list()
  ...
  await libFile.writeAsString([
    licence,
    for (final privateLib in privateLibs) "export 'src/generated_naver/$privateLib';",
  ].join('\n'));
}
...
Future<Lib> generatedElementsFromPage(
  ...
  for (final element in document.querySelectorAll('article.type-class')) {
    final fullName = element.querySelector('h1')!.text;
    final generate = generateClass;
    ...
    final generateResult = generate(fullName, element);
...
String? extractExtends(Element element) {
  final parent = element
      .querySelectorAll('dl.tsd-hierarchy > dd > a')
      .firstOrNull
      ?.text;
...
String? extractImplements(Element element) {
  final parent = element
      .querySelectorAll('dl.tsd-hierarchy > dd > span.tsd-signature-type')
      .lastOrNull
      ?.text;
  return parent == null ? null : translateType(parent);
}
...
Constructor? extractConstructor(
  ...
  final constructor = element.querySelector('.tsd-kind-constructor');
  if (constructor == null) return null;
  final name = constructor.querySelector('.tsd-signature-symbol')!.text;
  final parameters =
      extractParameters(constructor.querySelector('.tsd-parameters'));
  final doc = constructor.querySelector('.tsd-comment > p')?.text ?? '';
  final optionalIndex =
  ...
  return Constructor(
    ...
    doc: doc,
    parameters:
        parameters.takeWhile((p) => doc.indexOf(p.name) < optionalIndex).toList(),
    optionalParameters:
        parameters.skipWhile((p) => doc.indexOf(p.name) < optionalIndex).toList(),
  );
}
...
List<Method> extractMethods(
  ...
  bool isStatic,
    ...
    element
        .querySelectorAll(
            '.tsd-kind-method.tsd-parent-kind-class${isStatic ? '.tsd-is-static' : ''}')
        .map((e) {
      final name = e.querySelector('.tsd-signature-symbol')!.text;
      final parameters = extractParameters(e.querySelector('.tsd-parameters'));
      final returnType =
          e.querySelector('.tsd-signature-type')?.text ?? 'void';
      final doc = e.querySelector('.tsd-comment > p')?.text ?? '';
      final optionalIndex =
          doc.contains('[') ? doc.indexOf('[') : double.infinity;
      return Method(
        name: name,
...
List<Parameter> extractParameters(
  ...
  return element.querySelectorAll('li').map((e) {
    final name = e.querySelector('.tsd-parameter-name')!.text;
    final jsType = e.querySelector('.tsd-signature-type')!.text;
    final doc = e.querySelector('.tsd-comment > p')?.text ?? '';
    return Parameter(
      name: name,
      jsType: jsType,
...
List<Property> extractProperties(
  Element element,
    ...
    element.querySelectorAll('.tsd-kind-property').map((e) {
      final name = e.querySelector('.tsd-signature-symbol')!.text;
      final jsType = e.querySelector('.tsd-signature-type')!.text;
      final doc = e.querySelector('.tsd-comment > p')?.text ?? '';
      return Property(
...
List<({String name, List<({String name, String type})> parameters, String doc})>
  ...
  Element element,
        ...
        element.querySelectorAll('.tsd-kind-event').map((e) {
          final name = e.querySelector('.tsd-signature-symbol')!.text;
          final doc = e.querySelector('.tsd-comment > p')?.text ?? '';
          final parameters = e
              .querySelectorAll('.tsd-comment > .tsd-param-descr > li')
              .map((e) {
            final paramName = e.querySelector('span')!.text;
            final paramType =
                e.querySelector('.tsd-signature-type')?.text ?? 'any';
            return (name: paramName, type: translateType(paramType));
          }).toList();
          return (
            name: name,
            parameters: parameters,
...
List<ConstantValue> extractConstants(Element element) => element
    .querySelectorAll('.tsd-kind-property.tsd-is-static')
    .map((e) => ConstantValue(
          name: e.querySelector('.tsd-signature-symbol')!.text,
          doc: e.querySelector('.tsd-comment > p')?.text ?? '',
        ))
    .toList();
